...
<body class="layui-unselect lau-sign-body" style="padding-top: 0px " >
    <form  class="layui-form">
        ...
        <div>
            <label class="layui-form-label">选择教师id：</label>
            <div class="layui-input-block">
                <select lay-filter="teacher-data" id="teacher-data">
                </select>
            </div>
        </div>
        <div class="layui-form-item lau-sign-other" style="margin-top: 20px;text-align:center">
            <button type="button" class="layui-btn layui-btn-normal" lay-submit="" lay-filter="submitStuInfo" style="margin-right: 100px">提交</button>
        </div>
    </form>
</body>
<script>
    ...
    layui.use(['table','form'], function(){
        ...
        form.on('submit(submitStuInfo)', function(result) {
            ...
            $.ajax({
                url : '${pageContext.request.contextPath}/insertStuUser',
                data : data,
                type:'POST',
                dataType:'text',
    ...
    $(function () {
        ...
        setTimeout(function(){
            $.ajax({
                ...
                success: function (data) {
                    console.log("idddiidi:"+data.data)
                    $('#teacher-data').html('')
                    $('#teacher-data').append(new Option('请选择教师id','请选择教师id'))
                    $.each(data.data, function (index, item) {
                        $('#teacher-data').append(new Option(item.id, item.id));// 下拉菜单里添加元素
                    });
                    layui.form.render("select");
                }
            });
